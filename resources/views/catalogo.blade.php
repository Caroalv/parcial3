...
@section('content')
<div class="container"> <div class="row"> <div class="col-md-12"> <button class="btn btn-primary"
    onclick="verCarrito()"> Ver Carrito <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    fill="currentColor" class="bi bi-cart" viewBox="0 0 16 16"> <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12"> <div class="welcome-message"> <h1 class="text-center">Catálogo de venta</h1>
                <p class="text-center">Elige lo que deseas comprar</p> </div> </div>
                    </div> </div> <div class="container">
                <div class="row"> @foreach($productos as $producto) <!-- Producto -->
                    <div class="col-md-4">
                        <div class="card mb-4"> <div class="card-body"> <h5 class="card-title">Nombre: {{
                            $producto->nombre }}</h5>
                            <input type="text" id="nombre{{  $producto->id }}" disabled value="{{ $producto->nombre }}
                                    " hidden>
                            <h5 class="card-title" hidden>Id: {{ $producto->id }}</h5> <img src="{{ $producto->imagen
                                }}" alt="Imagen del producto" style="width: 100%; height; 100%; object-fit: cover;"> <p
                                class="card-text">Disponible: {{ $producto->stock }}</p>
                                <input type="number" id="stock{{ $producto->id }}" disabled value="{{ $producto->stock
                                    }}" hidden>
                                     <p class="card-text">Precio: ${{ $producto->precio }}</p> 
                                     <input type="text" id="precio{{  $producto->id }}" disabled value="{{ $producto->precio }}
                                    " hidden>          
                                     <label
